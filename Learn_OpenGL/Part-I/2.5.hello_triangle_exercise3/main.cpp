...
int main()
{
    ...
    /* set up vertex data and buffer(s) and configure vertex attributes
       ...
       the vertex attribute configuration remains the same (still one 3-float
    ...
    /* You can unbind the VAO afterwards so other VAO calls will not accidentally
       modify this VAO, but this rarely happens. Modifying other VAOs requires
       a call to glBindVertexArray anyways so we generally do not unbind VAOs
       nor VBOs when it is not dirrectly necessary */
    // glBindVertexArray(0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindVertexArray(VAOs[0]);
        /* see as we only have a single VAO there is no need to bind it
           every time, but we will do so to keep things more organized */
